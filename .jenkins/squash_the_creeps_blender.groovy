pipeline {
    ...
    stages {
        stage('Configured Docker nodes') {
            steps {
                script {
                    executeOnAll(
                            { Node node ->
                                if (node instanceof hudson.slaves.AbstractCloudSlave) {
                                    return false
                                }
...
def build() {
    dir(env.PROJECT_LOCATION) {
        ...
        stage('Import Assets') {
            ...
            for (asset in ['mob', 'player']) {
                def imported = findFiles(glob: ".godot/imported/${asset}.blend-*.scn")
                if (imported.length != 1) {
                    error "Expected one imported ${asset}.blend scene, found ${imported.length}"
                }
